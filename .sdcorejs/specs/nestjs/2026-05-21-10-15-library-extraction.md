...
description: Bóc tách be-masterdata/core-be/ → @sdcorejs/nestjs npm lib (mono-package, 8 sub-paths, fully neutral via DI strategies, NestJS 11 + TypeORM 0.3.x, dual ESM+CJS, TDD + 90% coverage)
approvedAt: 2026-05-21T10:15+07:00
approvedBy: nghiatt15@onemount.com
track: nestjs
module: (cross-cutting library extraction)
entity: (none — framework library, not entity scaffolding)
sourceSpecPath: .sdcorejs/docs/nestjs/2026-05-21-10-00-library-extraction-spec.md
---

# Bóc tách `@sdcorejs/nestjs` từ `be-masterdata/core-be/` — Approved Spec
...
User approved on first presentation (no user-requested iterations). Agent self-review applied 4 fixes BEFORE presenting to user — these reflect inferred style preferences worth mirroring in future specs:

1. **Drop deferred phrasing in acceptance criteria** — original `/tenancy` AC had "chi tiết enforcement điểm này được plan quyết định". Replaced with concrete list of methods (`paging/all/search/detail` for filter inject, `create/import` for auto-fill). Style signal: *spec must commit to concrete behavior; defer architecture decisions belong in `out-of-scope` not in `acceptance-criteria`*.

2. **Single-point-of-enforcement preference** — original architecture mentioned both `TenancyInterceptor` AND repository-level enforcement. Dropped `TenancyInterceptor` entirely; `BaseRepository` is sole enforcer. Added explicit AC `No TenancyInterceptor exists`. Style signal: *don't duplicate enforcement mechanisms; pick one canonical layer; spec must call out what does NOT exist when it's a meaningful design choice*.

3. **Clean duplicate decorator location** — original file structure put `@TenantScoped` in both `/orm/decorators/` (primary) and `/tenancy/decorators/` (re-export with a separate file). Simplified to `/orm/decorators/` only, with `/tenancy/index.ts` doing barrel re-export instead. Style signal: *avoid duplicate definition files; barrel re-export is enough for ergonomic imports*.

4. **Strategy contracts narrative consistency** — strategy contracts section was still narratively describing `TenancyInterceptor` even after sub-path table dropped it. Rewrote to match the single-point-of-enforcement decision. Style signal: *when changing an architecture decision, hunt all 3 places it appears (sub-path table, narrative, AC, file structure) and align them*.

No user-requested changes during the review gate itself — these were caught by agent self-review prior to presentation.

## Skill provenance

`03-write-spec` → `04-review-spec` (approved on attempt 1 / 3, with 4 agent self-review fixes applied before user saw the spec)
